...
class Client:
    ...
    def send_message(self):
        while True:
            client_input = input('')
            enc_message = index_key.encrypt(client_input.encode('utf-8').hex(), index_key.key(main_key(2)))
            self.socket.send(enc_message.encode())

    def receive_message(self):
        while True:
            enc_message = self.socket.recv(1024).decode()
            server_inbox = bytes.fromhex(index_key.decrypt(enc_message, index_key.key(main_key(1)))).decode('utf-8')
